...
Configured with Hetzner daily backups.
...
- rsync: Hourly backup to block storage
- pgBackRest: Hourly PostgreSQL backups to block storage
...
- Allow 80/tcp on localhost
- Allow 22/tcp from private network
- Deny all other incoming

Connect to the monitoring server using SSH forwarding, like this:
